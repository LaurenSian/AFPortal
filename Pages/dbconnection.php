...
$sqlshiftcurrent = 'SELECT DATE_FORMAT(CONVERT(Shift_Start, DATE), "%a %e-%b-%y") AS Shift_StartDate, DATE_FORMAT(CONVERT(Shift_Start, TIME), "%H:%i") AS Shift_StartTime, DATE_FORMAT(CONVERT(Shift_End, TIME), "%H:%i") AS Shift_EndTime, Site_Name, Shift_Hours, CONCAT("£", Shift_Payrate) AS Shift_Payrate, Assignment_Confirmed, Site_Address, Site_City, Site_Postcode
FROM Site
...
WHERE Employee.Employee_ID = "AF001" AND Shift_End >= NOW()
